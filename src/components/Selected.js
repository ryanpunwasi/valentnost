...
const Selected = (props) => {
  ...
  if(!props.element) {
    return(
      <div className='selected-container'>
        <div className='highlighted-element'>
          <span><i className="selected-placeholder fa-solid fa-flask"></i></span>
        </div>
      </div>
    );
  } else {
    ...
    return(
      <div className='selected-container'>
        <div className={`highlighted-element highlight-${apiGroupToClassName(renderGroup(props.element.name))}`}>
          <span className='selected-symbol'>{props.element.symbol}</span>
          <span className='selected-name'>{props.element.name}</span>
          <span className='selected-atomic-number'>{props.element.atomicNumber}</span>
          <span className='selected-state'>{capitalize(renderState(props.element.standardState))}</span>
          <span className='selected-group'>{renderGroup(props.element.name)}</span>
        </div>
      </div>
    );
  }
